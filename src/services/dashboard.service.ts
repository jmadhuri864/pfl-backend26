...
async getProcurementSourceWise(userId: string): Promise<any> {
  const teamMembers = await this.workflowHierarchyRepo.find({
    where: {
      ancestor: { id: userId },
      department: DepartmentEnum.PURCHASE,
  ...
  const [vendorData, farmerData] = await Promise.all([
    this.getProcurementMetrics(memberIds, "vendor", currentMonth, currentYear),
    this.getProcurementMetrics(memberIds, "farmer", currentMonth, currentYear)
  ]);
...
// ─── helper: get descendant employee IDs for a team leader ────────────────
  ...
  //   };
  // }
  async getFarmerRegistrationOverviewOfTeam(teamLeaderId: string) {
...
async getVendorRegistrationOverviewOfTeam(teamLeaderId: string) {
   const teamMembers = await this.workflowHierarchyRepo
  .createQueryBuilder("wh")
  .select("descendant.id", "id")
  .leftJoin("wh.descendant", "descendant")
...
async getCustomerRegistrationOverviewOfTeam(teamLeaderId: string) {
   const teamMembers = await this.workflowHierarchyRepo
  .createQueryBuilder("wh")
  .select("descendant.id", "id")
  .leftJoin("wh.descendant", "descendant")
