...
console.log("Creating Dump Register with data:", data);
      //TODO: Check approval flow is exit or not for logged user

    //  const approvalFlowExit = this.approvalFlowService.findApprovalFlowForLoggedUser(data.requestedBy, 'dump-register')
...
console.log("productIds:", productIds);
      const dumpRegister = this.dumpRegisterRepository.create({...data,
      ...
      const document = await this.documentService.createDocument({
       ...
       // lastActionBy: { id: Array.isArray(savedDumpRegister) ? (savedDumpRegister[0] as DumpRegister)?.requestedBy : (savedDumpRegister as DumpRegister).requestedBy },
        lastActionBy: {id: data.requestedBy},
        document_type_id: Array.isArray(savedDumpRegister) ? (savedDumpRegister[0] as DumpRegister)?.id : (savedDumpRegister as DumpRegister).id
      }, );

      console.log("Document created with ID:", document.id);
      console.log("document type is ",document.type);
